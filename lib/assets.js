...
import { getMaintenanceHistoryForAsset } from "@/lib/maintenance";
...
export async function getAssetWithHistory(id) {
  ...
  const maintenance = await getMaintenanceHistoryForAsset(id);

  return { asset: assetRes.rows[0], history: historyRes.rows, maintenance };
}
